...
from keyboards.inline import keyboard_spam_cancel_dialog, keyboard_spam_question_dialog
...
async def message_all_wait_msg(message: Message, bot: Bot, state: FSMContext) -> None:
	# try:
	wait_msg = await message.answer('Отправьте сообщение:', reply_markup=keyboard_spam_cancel_dialog)  # mb need autoremove..
...
async def message_all_wait_accept_send(message: Message, bot: Bot, state: FSMContext) -> None:
	try:
		...
		await message.answer('Подтвердите действие:', reply_markup=keyboard_spam_question_dialog)
